(function(){
  ...
  // A '80s convertible sportfisher (Bertram 54 / Viking 55 style) runs a
  // white fiberglass hull and superstructure with a bold accent stripe near
  // the waterline — never the hull's base color itself, so it always reads
  // clearly against blue water regardless of which stripe color is picked.
  // Rail/tower/hardtop trim stays a fixed brushed-silver rather than hue-
  // hashed, since that hardware is metal on the real boats.
  var HULL_HUE_CHOICES = [210, 355, 38, 150]; // navy, red, gold, forest green stripe
  var TRIM_HUE_CHOICES = [205, 210, 200];
  function initBoat(){
  ...
  // The hull is a lengthwise side-profile of an '80s convertible sportfisher
  // (Bertram 54 / Viking 55 territory): long and low, a raked bow, a flat
  // transom, most of its height carried by the cabin/flybridge/tower stacked
  // on top rather than the hull itself. Built from named stations (f = -1
  // stern .. 1 bow) giving {topDy, bottomDy} relative to "by", linearly
  // interpolated per column — long enough to dwarf even the biggest fish.
  var HULL_HL = 78; // half-length in px; the hull is HULL_HL*2 px long
  var HULL_STATIONS = [
    { f: -1.00, topDy: -7,  bottomDy: -1  },
    { f: -0.75, topDy: -8,  bottomDy: 0   },
    { f: -0.40, topDy: -9,  bottomDy: 0   },
    { f: -0.10, topDy: -9,  bottomDy: -1  },
    { f:  0.20, topDy: -10, bottomDy: -2  },
    { f:  0.50, topDy: -11, bottomDy: -4  },
    { f:  0.75, topDy: -13, bottomDy: -8  },
    { f:  0.92, topDy: -16, bottomDy: -14 },
    { f:  1.00, topDy: -19, bottomDy: -19 }
  ];
  ...
  // Sportfishing happens off the aft cockpit, not the bow — the cabin,
  // flybridge and tower sit forward of amidships instead.
  var STERN_STAND_OFF = 55; // px aft of center the fisherman stands, in the open cockpit
  ...
  function boatRodTip(){
    ...
    var deckY = by + hullTopDyAt(-STERN_STAND_OFF);
    var px = bx - boat.faceDir*STERN_STAND_OFF;
  ...
  function drawBoat(){
    ...
    /* hull: raked bow, flat transom, low sleek freeboard — white fiberglass
       body with a bold accent stripe near the waterline and a dark
       antifouled keel, plus a two-tone panel-line alternation on the body */
    for(var c=-HULL_HL; c<HULL_HL; c++){
      ...
      var h = botY - topY + 1;
      if(h <= 1){
        ctx.fillStyle = 'hsl(' + trimHue + ' 18% 85%)';
        ctx.fillRect(colX, topY, 1, Math.max(1, h));
        continue;
      }
      ctx.fillStyle = 'hsl(' + trimHue + ' 18% 85%)';
      ctx.fillRect(colX, topY, 1, 1); // rail
      var stripeOn = h >= 5;
      var bodyBottom = stripeOn ? botY-2 : botY-1;
      if(bodyBottom >= topY+1){
        ctx.fillStyle = (Math.abs(c) % 6 < 3) ? 'hsl(200 10% 91%)' : 'hsl(200 8% 85%)';
        ctx.fillRect(colX, topY+1, 1, bodyBottom-(topY+1)+1);
      }
      if(stripeOn){
        ctx.fillStyle = 'hsl(' + hullHue + ' 55% 42%)';
        ctx.fillRect(colX, botY-1, 1, 1);
      }
      ctx.fillStyle = 'hsl(210 20% 14%)';
      ctx.fillRect(colX, botY, 1, 1); // antifouled keel
    }

    /* cabin, forward of amidships, with a raked windshield toward the bow.
       A dark roofline pixel on every column keeps the white cabin readable
       against a pale sky regardless of time of day. */
    var cabinC0 = 2, cabinC1 = 42, rakeStart = 32;
    var cabinDeckY = Math.round(by + hullTopDyAt((cabinC0+cabinC1)/2));
    var cabinRoofY = cabinDeckY - 12;
    for(var cc=cabinC0; cc<=cabinC1; cc++){
      var cTop = cabinRoofY;
      if(cc > rakeStart) cTop = cabinRoofY + Math.round(((cc-rakeStart)/(cabinC1-rakeStart)) * 9);
      ctx.fillStyle = 'hsl(205 30% 24%)';
      ctx.fillRect(bx + cc*faceDir, cTop, 1, 1);
      ctx.fillStyle = ((cc-cabinC0) % 2 === 0) ? 'hsl(200 10% 90%)' : 'hsl(200 8% 84%)';
      ctx.fillRect(bx + cc*faceDir, cTop+1, 1, cabinDeckY-cTop-1);
    }
    ctx.fillStyle = 'hsl(205 35% 20%)'; // window band
    for(cc=cabinC0+3; cc<=rakeStart; cc++) ctx.fillRect(bx + cc*faceDir, cabinRoofY+3, 1, 4);

    /* flybridge with hardtop, set back on the cabin roof */
    var flyC0 = 10, flyC1 = 28, flyRoofY = cabinRoofY - 7;
    ctx.fillStyle = 'hsl(' + trimHue + ' 30% 42%)';
    for(cc=flyC0-1; cc<=flyC1+1; cc++) ctx.fillRect(bx + cc*faceDir, flyRoofY, 1, 1); // hardtop lip
    for(cc=flyC0; cc<=flyC1; cc++){
      ctx.fillStyle = 'hsl(200 8% 92%)';
      ctx.fillRect(bx + cc*faceDir, flyRoofY+1, 1, cabinRoofY-flyRoofY-1);
    }

    /* tuna tower above the flybridge, with a spotting platform on top */
    var towerC = 18, towerX = bx + towerC*faceDir;
    var towerTopY = flyRoofY - 13;
    ctx.fillStyle = 'hsl(' + trimHue + ' 15% 55%)';
    for(var ty=towerTopY; ty<=flyRoofY; ty++){
      ctx.fillRect(towerX-3, ty, 1, 1);
      ctx.fillRect(towerX+3, ty, 1, 1);
    }
    ctx.fillStyle = 'hsl(' + trimHue + ' 25% 40%)';
    ctx.fillRect(towerX-4, towerTopY, 9, 1);
    ctx.fillStyle = 'hsl(200 8% 90%)';
    ctx.fillRect(towerX-4, towerTopY+1, 9, 1);

    /* outrigger, angled up and aft over the open cockpit */
    var outEndC = -50;
    var outStartX = towerX, outStartY = towerTopY+2;
    var outEndX = bx + outEndC*faceDir, outEndY = towerTopY-20;
    ctx.fillStyle = 'hsl(210 15% 55%)';
    for(var oi=0; oi<=22; oi++){
      var ox = outStartX + (outEndX-outStartX)*(oi/22);
      var oy = outStartY + (outEndY-outStartY)*(oi/22);
      ctx.fillRect(Math.round(ox), Math.round(oy), 1, 1);
    }

    /* fighting chair bolted in the cockpit */
    var chairC = -32, chairX = bx + chairC*faceDir, chairDeckY = Math.round(by + hullTopDyAt(chairC));
    ctx.fillStyle = 'hsl(0 45% 30%)';
    ctx.fillRect(chairX-2, chairDeckY-4, 5, 2);
    ctx.fillStyle = 'hsl(210 10% 40%)';
    ctx.fillRect(chairX-1, chairDeckY-2, 2, 2);

    var reelPhase = reelAnim ? reelAnim.phase : null;

    /* fisherman, standing in the aft cockpit — leans away from the boat
       when the fish makes a hard run */
    var lean = reelPhase === 'pull'
      ? faceDir * Math.round((reelAnim.fightIntensity||0) * 2)
      : 0;
    var px = bx - faceDir*STERN_STAND_OFF + lean;
  ...
  function victoryHoldPos(f){
    ...
    var by = Math.round(SKY_H + waveY(bx) - 1);
    var deckY = by + hullTopDyAt(-STERN_STAND_OFF);
    var raiseX = bx - boat.faceDir*(STERN_STAND_OFF+1);
